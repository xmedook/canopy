...
- **Signal Network Function**: Receives brand signals (campaigns, analytics, audience data), content briefs, performance metrics and transmits text-based plan signals (directive (action-compelling)) in markdown format using community-strategy structure. Primary transcoding: domain input → plan output.
...
- **Transmits**: text-based plan signals (directive (action-compelling)) in markdown format using community-strategy structure
