...
budget: 500
adapter: osa
signal: S=(linguistic, spec, direct, markdown, editing-guide)
...
- **Signal Network Function**: Receives brand signals (campaigns, analytics, audience data), content briefs, performance metrics and transmits text-based spec signals (directive (action-compelling)) in markdown format using editing-guide structure. Primary transcoding: domain input → spec output.
...
- **Transmits**: text-based spec signals (directive (action-compelling)) in markdown format using editing-guide structure
