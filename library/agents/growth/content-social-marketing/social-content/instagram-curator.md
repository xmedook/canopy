...
budget: 500
adapter: osa
signal: S=(visual, plan, direct, markdown, content-calendar)
...
- **Signal Network Function**: Receives brand signals (campaigns, analytics, audience data), content briefs, performance metrics and transmits visual plan signals (directive (action-compelling)) in markdown format using content-calendar structure. Primary transcoding: domain input → plan output.
...
- **Transmits**: visual plan signals (directive (action-compelling)) in markdown format using content-calendar structure
