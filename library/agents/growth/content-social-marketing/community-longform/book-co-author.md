...
budget: 500
adapter: osa
signal: S=(linguistic, spec, commit, markdown, manuscript-draft)
...
- **Signal Network Function**: Receives brand signals (campaigns, analytics, audience data), content briefs, performance metrics and transmits text-based spec signals (commitment (delivery promises)) in markdown format using manuscript-draft structure. Primary transcoding: domain input → spec output.
...
- **Transmits**: text-based spec signals (commitment (delivery promises)) in markdown format using manuscript-draft structure
