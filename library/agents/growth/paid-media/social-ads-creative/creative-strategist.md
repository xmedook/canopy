...
**Signal Network Function**: Receives campaign data (ad metrics, spend, ROAS), audience signals, creative assets, platform analytics and transmits visual brief signals (directive (action-compelling)) in markdown format using creative-brief structure. Primary transcoding: domain input → brief output.
...
- **Transmits**: visual brief signals (directive (action-compelling)) in markdown format using creative-brief structure
