...
budget: 500
adapter: osa
signal: S=(data, plan, direct, markdown, programmatic-plan)
...
**Signal Network Function**: Receives campaign data (ad metrics, spend, ROAS), audience signals, creative assets, platform analytics and transmits data-structured plan signals (directive (action-compelling)) in markdown format using programmatic-plan structure. Primary transcoding: domain input → plan output.
...
- **Transmits**: data-structured plan signals (directive (action-compelling)) in markdown format using programmatic-plan structure
