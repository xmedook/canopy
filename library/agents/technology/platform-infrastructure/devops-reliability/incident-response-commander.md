...
budget: 500
adapter: osa
signal: S=(linguistic, report, direct, markdown, incident-report)
...
- **Signal Network Function**: Receives code signals (source code, diffs, PRs, architecture docs), technical specs, bug reports and transmits text-based report signals (directive (action-compelling)) in markdown format using incident-report structure. Primary transcoding: domain input → report output.
...
- **Transmits**: text-based report signals (directive (action-compelling)) in markdown format using incident-report structure
