...
reportsTo: security-engineer
budget: 500
adapter: osa
signal: S=(data, report, inform, markdown, threat-detection-rules)
...
- **Signal Network Function**: Receives code signals (source code, diffs, PRs, architecture docs), technical specs, bug reports and transmits data-structured report signals (informational) in markdown format using threat-detection-rules structure. Primary transcoding: domain input → report output.
...
- **Transmits**: data-structured report signals (informational) in markdown format using threat-detection-rules structure
