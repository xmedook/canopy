...
- **Signal Network Function**: Receives code signals (source, diffs), test results, bug reports, coverage metrics, quality gates and transmits text-based report signals (decision-oriented) in markdown format using tool-evaluation structure. Primary transcoding: domain input → report output.
...
- **Transmits**: text-based report signals (decision-oriented) in markdown format using tool-evaluation structure
