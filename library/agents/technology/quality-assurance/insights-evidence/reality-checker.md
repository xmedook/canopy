...
budget: 3000
adapter: osa
signal: S=(linguistic, report, decide, markdown, agent-definition)
...
- **Signal Network Function**: Receives code signals (source, diffs), test results, bug reports, coverage metrics, quality gates and transmits text-based report signals (decision-oriented) in markdown format using agent-definition structure. Primary transcoding: domain input → report output.
...
- **Transmits**: text-based report signals (decision-oriented) in markdown format using agent-definition structure
