...
reportsTo: api-tester
budget: 3000
adapter: osa
signal: S=(code, report, inform, markdown, agent-definition)
...
- **Signal Network Function**: Receives code signals (source, diffs), test results, bug reports, coverage metrics, quality gates and transmits code-based report signals (informational) in markdown format using agent-definition structure. Primary transcoding: domain input → report output.
...
- **Transmits**: code-based report signals (informational) in markdown format using agent-definition structure
