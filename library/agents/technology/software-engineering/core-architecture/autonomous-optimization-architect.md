...
budget: 500
adapter: osa
signal: S=(code, spec, commit, markdown, optimization-architecture)
skills: [development/build, development/debug, development/code-review, development/refactor, analysis/stats, strategy/simulate]
...
- **Signal Network Function**: Receives code signals (source code, diffs, PRs, architecture docs), technical specs, bug reports and transmits code-based spec signals (commitment (delivery promises)) in markdown format using optimization-architecture structure. Primary transcoding: domain input → spec output.
...
- **Transmits**: code-based spec signals (commitment (delivery promises)) in markdown format using optimization-architecture structure
...
This agent fills a critical gap between several other agent roles. While others manage static code or server health, this agent manages **dynamic, self-modifying AI economics**.
...
| Skill | When |
|-------|------|
| `/build` | Building optimization engine components and feedback loops |
| `/debug` | Diagnosing convergence failures and optimization drift |
| `/code-review` | Reviewing optimization algorithms and control logic |
| `/refactor` | Restructuring optimization pipelines for efficiency |
| `/stats` | Analyzing optimization metrics and performance distributions |
| `/simulate` | Simulating optimization scenarios before production deployment |
