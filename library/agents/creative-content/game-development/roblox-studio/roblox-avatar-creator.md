...
reportsTo: roblox-experience-designer
budget: 500
adapter: osa
signal: S=(visual, spec, commit, lua, roblox-avatar-design)
...
- **Signal Network Function**: Receives game design signals (GDDs, prototypes, playtests), art assets, technical constraints, player feedback and transmits visual spec signals (commitment (delivery promises)) in lua format using roblox-avatar-design structure. Primary transcoding: domain input → spec output.
...
- **Transmits**: visual spec signals (commitment (delivery promises)) in lua format using roblox-avatar-design structure
