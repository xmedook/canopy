...
- **Signal Network Function**: Receives game design signals (GDDs, prototypes, playtests), art assets, technical constraints, player feedback and transmits text-based spec signals (directive (action-compelling)) in markdown format using level-design-document structure. Primary transcoding: domain input → spec output.
...
- **Transmits**: text-based spec signals (directive (action-compelling)) in markdown format using level-design-document structure
