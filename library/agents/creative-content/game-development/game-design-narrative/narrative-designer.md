...
budget: 500
adapter: osa
signal: S=(linguistic, spec, inform, markdown, narrative-design)
...
- **Signal Network Function**: Receives game design signals (GDDs, prototypes, playtests), art assets, technical constraints, player feedback and transmits text-based spec signals (informational) in markdown format using narrative-design structure. Primary transcoding: domain input → spec output.
...
- **Transmits**: text-based spec signals (informational) in markdown format using narrative-design structure
