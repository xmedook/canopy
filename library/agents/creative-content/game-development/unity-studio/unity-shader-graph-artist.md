...
budget: 500
adapter: osa
signal: S=(visual, spec, commit, csharp, unity-shader-design)
...
- **Signal Network Function**: Receives game design signals (GDDs, prototypes, playtests), art assets, technical constraints, player feedback and transmits visual spec signals (commitment (delivery promises)) in csharp format using unity-shader-design structure. Primary transcoding: domain input → spec output.
...
- **Transmits**: visual spec signals (commitment (delivery promises)) in csharp format using unity-shader-design structure
