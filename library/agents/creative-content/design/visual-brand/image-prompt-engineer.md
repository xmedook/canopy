...
budget: 500
adapter: osa
signal: S=(linguistic, spec, direct, markdown, prompt-template)
...
- **Signal Network Function**: Receives visual signals (mockups, brand assets, style guides), design briefs, user research, accessibility reports and transmits text-based spec signals (directive (action-compelling)) in markdown format using prompt-template structure. Primary transcoding: domain input → spec output.
...
- **Transmits**: text-based spec signals (directive (action-compelling)) in markdown format using prompt-template structure
