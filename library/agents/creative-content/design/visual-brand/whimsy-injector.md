...
budget: 500
adapter: osa
signal: S=(linguistic, brief, express, markdown, creative-direction)
...
- **Signal Network Function**: Receives visual signals (mockups, brand assets, style guides), design briefs, user research, accessibility reports and transmits text-based brief signals (perspective-sharing) in markdown format using creative-direction structure. Primary transcoding: domain input → brief output.
...
- **Transmits**: text-based brief signals (perspective-sharing) in markdown format using creative-direction structure
