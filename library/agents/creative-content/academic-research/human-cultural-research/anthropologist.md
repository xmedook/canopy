...
- **Signal Network Function**: Receives research signals (papers, datasets, citations), methodology specs, peer review feedback and transmits text-based report signals (informational) in markdown format using ethnographic-analysis structure. Primary transcoding: domain input → report output.
...
- **Transmits**: text-based report signals (informational) in markdown format using ethnographic-analysis structure
