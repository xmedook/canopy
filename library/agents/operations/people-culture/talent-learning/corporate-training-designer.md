...
reportsTo: recruitment-specialist
budget: 500
adapter: osa
signal: S=(linguistic, spec, direct, markdown, training-curriculum)
skills: [content/write, content/edit, content/slides, strategy/plan, learning/learn]
...
- **Signal Network Function**: Receives domain-specific signals varying by specialization — see role definition for specific input types and transmits text-based spec signals (directive (action-compelling)) in markdown format using training-curriculum structure. Primary transcoding: domain input → spec output.
...
- **Transmits**: text-based spec signals (directive (action-compelling)) in markdown format using training-curriculum structure
...
| Skill | When |
|-------|------|
| `/write` | Creating training materials and course content |
| `/edit` | Refining training content for clarity and engagement |
| `/slides` | Building training presentations and visual aids |
| `/plan` | Designing training programs and learning paths |
| `/learn` | Capturing training effectiveness patterns |
