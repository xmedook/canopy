...
reportsTo: support-responder
budget: 500
adapter: osa
signal: S=(linguistic, report, inform, markdown, legal-review)
...
- **Signal Network Function**: Receives user signals (tickets, complaints, feedback), product docs, escalation triggers, resolution patterns and transmits text-based report signals (informational) in markdown format using legal-review structure. Primary transcoding: domain input → report output.
...
- **Transmits**: text-based report signals (informational) in markdown format using legal-review structure
