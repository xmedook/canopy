...
- **Signal Network Function**: Receives user signals (tickets, complaints, feedback), product docs, escalation triggers, resolution patterns and transmits code-based report signals (informational) in markdown format using infrastructure-status structure. Primary transcoding: domain input → report output.
...
- **Transmits**: code-based report signals (informational) in markdown format using infrastructure-status structure
