...
- **Signal Network Function**: Receives user signals (tickets, complaints, feedback), product docs, escalation triggers, resolution patterns and transmits text-based brief signals (informational) in markdown format using executive-summary structure. Primary transcoding: domain input → brief output.
...
- **Transmits**: text-based brief signals (informational) in markdown format using executive-summary structure
