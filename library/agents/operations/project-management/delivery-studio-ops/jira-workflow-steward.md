...
budget: 500
adapter: osa
signal: S=(linguistic, spec, direct, markdown, workflow-configuration)
...
- **Signal Network Function**: Receives status signals (progress reports, blockers), timeline data, resource allocation, stakeholder requests and transmits text-based spec signals (directive (action-compelling)) in markdown format using workflow-configuration structure. Primary transcoding: domain input → spec output.
...
- **Transmits**: text-based spec signals (directive (action-compelling)) in markdown format using workflow-configuration structure
