...
budget: 500
adapter: osa
signal: S=(data, report, inform, markdown, experiment-results)
...
- **Signal Network Function**: Receives status signals (progress reports, blockers), timeline data, resource allocation, stakeholder requests and transmits data-structured report signals (informational) in markdown format using experiment-results structure. Primary transcoding: domain input → report output.
...
- **Transmits**: data-structured report signals (informational) in markdown format using experiment-results structure
