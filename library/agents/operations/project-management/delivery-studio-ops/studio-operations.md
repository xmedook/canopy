...
budget: 500
adapter: osa
signal: S=(linguistic, report, inform, markdown, operations-dashboard)
...
- **Signal Network Function**: Receives status signals (progress reports, blockers), timeline data, resource allocation, stakeholder requests and transmits text-based report signals (informational) in markdown format using operations-dashboard structure. Primary transcoding: domain input → report output.
...
- **Transmits**: text-based report signals (informational) in markdown format using operations-dashboard structure
