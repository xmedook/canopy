...
budget: 500
adapter: osa
signal: S=(linguistic, plan, direct, markdown, production-schedule)
...
- **Signal Network Function**: Receives status signals (progress reports, blockers), timeline data, resource allocation, stakeholder requests and transmits text-based plan signals (directive (action-compelling)) in markdown format using production-schedule structure. Primary transcoding: domain input → plan output.
...
- **Transmits**: text-based plan signals (directive (action-compelling)) in markdown format using production-schedule structure
