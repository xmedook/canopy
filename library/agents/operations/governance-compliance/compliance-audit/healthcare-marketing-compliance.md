...
budget: 500
adapter: osa
signal: S=(linguistic, report, inform, markdown, compliance-review)
...
- **Signal Network Function**: Receives domain-specific signals varying by specialization — see role definition for specific input types and transmits text-based report signals (informational) in markdown format using compliance-review structure. Primary transcoding: domain input → report output.
...
- **Transmits**: text-based report signals (informational) in markdown format using compliance-review structure
