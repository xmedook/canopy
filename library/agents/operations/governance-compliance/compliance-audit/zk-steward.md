...
budget: 500
adapter: osa
signal: S=(linguistic, spec, commit, markdown, knowledge-graph)
...
- **Signal Network Function**: Receives knowledge artifacts, note connections, domain queries, and validation requests and transmits spec and inform signals in markdown format. Primary transcoding: knowledge input → structured note output.
...
## Signal Network
- **Receives**: knowledge artifacts, note connections, domain queries, validation requests
- **Transmits**: spec, inform signals (markdown)
- **Transcoding** (tools as signal converters):
  - Read: file → linguistic
  - Write: linguistic → persistent artifact
