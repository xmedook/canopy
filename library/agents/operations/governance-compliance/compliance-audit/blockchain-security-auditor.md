...
budget: 500
adapter: osa
signal: S=(code, report, inform, markdown, security-audit)
...
- **Signal Network Function**: Receives domain-specific signals varying by specialization — see role definition for specific input types and transmits code-based report signals (informational) in markdown format using security-audit structure. Primary transcoding: domain input → report output.
...
- **Transmits**: code-based report signals (informational) in markdown format using security-audit structure
