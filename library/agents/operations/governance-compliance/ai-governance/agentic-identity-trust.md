...
budget: 500
adapter: osa
signal: S=(linguistic, spec, commit, markdown, identity-architecture)
...
- **Signal Network Function**: Receives domain-specific signals varying by specialization — see role definition for specific input types and transmits text-based spec signals (commitment (delivery promises)) in markdown format using identity-architecture structure. Primary transcoding: domain input → spec output.
...
- **Transmits**: text-based spec signals (commitment (delivery promises)) in markdown format using identity-architecture structure
