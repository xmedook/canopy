...
budget: 500
adapter: osa
signal: S=(code, spec, commit, typescript, mcp-server)
...
- **Signal Network Function**: Receives domain-specific signals varying by specialization — see role definition for specific input types and transmits code-based spec signals (commitment (delivery promises)) in typescript format using mcp-server structure. Primary transcoding: domain input → spec output.
...
- **Transmits**: code-based spec signals (commitment (delivery promises)) in typescript format using mcp-server structure
