...
budget: 500
adapter: osa
signal: S=(data, plan, direct, markdown, supply-chain-plan)
...
- **Signal Network Function**: Receives domain-specific signals varying by specialization — see role definition for specific input types and transmits data-structured plan signals (directive (action-compelling)) in markdown format using supply-chain-plan structure. Primary transcoding: domain input → plan output.
...
- **Transmits**: data-structured plan signals (directive (action-compelling)) in markdown format using supply-chain-plan structure
