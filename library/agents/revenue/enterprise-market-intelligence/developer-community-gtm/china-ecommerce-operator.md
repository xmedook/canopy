...
reportsTo: developer-advocate
budget: 500
adapter: osa
signal: S=(linguistic, plan, direct, markdown, ecommerce-strategy)
...
- **Signal Network Function**: Receives brand signals (campaigns, analytics, audience data), content briefs, performance metrics and transmits text-based plan signals (directive (action-compelling)) in markdown format using ecommerce-strategy structure. Primary transcoding: domain input → plan output.
...
### Signal Network
- **Receives**: brand signals (campaigns, analytics, audience data), content briefs, performance metrics
- **Transmits**: text-based plan signals (directive (action-compelling)) in markdown format using ecommerce-strategy structure
