...
budget: 500
adapter: osa
signal: S=(linguistic, plan, inform, markdown, study-plan)
...
- **Signal Network Function**: Receives domain-specific signals varying by specialization — see role definition for specific input types and transmits text-based plan signals (informational) in markdown format using study-plan structure. Primary transcoding: domain input → plan output.
...
- **Transmits**: text-based plan signals (informational) in markdown format using study-plan structure
