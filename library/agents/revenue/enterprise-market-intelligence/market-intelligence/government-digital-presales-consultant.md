...
budget: 500
adapter: osa
signal: S=(linguistic, proposal, direct, markdown, government-proposal)
...
- **Signal Network Function**: Receives domain-specific signals varying by specialization — see role definition for specific input types and transmits text-based proposal signals (directive (action-compelling)) in markdown format using government-proposal structure. Primary transcoding: domain input → proposal output.
...
- **Transmits**: text-based proposal signals (directive (action-compelling)) in markdown format using government-proposal structure
