...
- **Signal Network Function**: Receives domain-specific signals varying by specialization — see role definition for specific input types and transmits code-based spec signals (commitment (delivery promises)) in markdown format using salesforce-architecture structure. Primary transcoding: domain input → spec output.
...
- **Transmits**: code-based spec signals (commitment (delivery promises)) in markdown format using salesforce-architecture structure
