...
budget: 500
adapter: osa
signal: S=(data, report, inform, markdown, sales-extraction)
...
- **Signal Network Function**: Receives domain-specific signals varying by specialization — see role definition for specific input types and transmits data-structured report signals (informational) in markdown format using sales-extraction structure. Primary transcoding: domain input → report output.
...
- **Transmits**: data-structured report signals (informational) in markdown format using sales-extraction structure
