...
## Agents as Signal Network Nodes

Every agent is a **node** in the Signal Network. When you design an agent, you are
defining a signal processing unit — what it can receive, how it transforms signals,
and what it transmits. The agent format standard (see `protocol/agent-format.md`)
encodes this completely:

**Receiving capacity** (what signals can this agent decode?):
- Mode competence: text, code, data, visual? Determined by tools + domain expertise
- Genre competence: specs, reports, briefs, scorecards? Declared in `signal:` field and Communication Style
- Format handling: markdown, JSON, PDF, video? Determined by tools (signal transcoders)

**Processing capability** (how does this agent transform signals?):
- Domain expertise: Core Mission + Process/Methodology
- Decision boundaries: Critical Rules (variety attenuation)
- Tool access: signal transcoders for mode/format conversion
- Skill repertoire: genre templates for efficient encoding

**Transmitting pattern** (what signals does this agent produce?):
- Default encoding: `signal: S=(mode, genre, type, format, structure)`
- Output templates: Deliverable Templates section
- Receiver calibration: Communication Style section

When you design an agent, resolve ALL THREE. An agent with strong processing
capability but weak receiving capacity will fail when it encounters unfamiliar
signal modes. An agent with broad receiving capacity but no deliverable templates
will produce unstructured noise.
...
See `agents/revenue/sales/deal-account/deal-strategist.md` for the full body -- it includes MEDDPICC
...
### Signal Theory Validation

Before finalizing an agent definition, verify:

- [ ] **Ashby (variety)**: Does the agent's genre competence (signal: field + tools + skills) match the variety of tasks it will encounter? An agent assigned to handle customer issues needs more variety than one that only writes unit tests.
- [ ] **Shannon (capacity)**: Is the agent's context_tier appropriate? An L0 agent receiving full-complexity tasks will be bandwidth-overloaded. An L2 agent doing simple lookups wastes capacity.
- [ ] **Beer (structure)**: Does the agent have coherent identity across all 8 body sections? A mismatch between Core Mission and Critical Rules indicates a structural failure.
- [ ] **Wiener (feedback)**: Does the agent have Success Metrics? Without measurable feedback targets, there's no way to detect alignment drift over time.
- [ ] **Tool coverage**: Can the agent transcode all signal modes/formats it will receive? If it handles PDFs, does it have a file-read tool? If it handles images, does it have vision capabilities?
- [ ] **Skill coverage**: Does the agent have skills for its recurring tasks? Repeated tasks without skills means the agent reinvents the process every time (stuck in System 2).
- [ ] **Routing path**: Does `reportsTo` point to a valid escalation target? Signals the agent can't handle need somewhere to go.

---

## Signal Theory Position

Agent design IS signal architecture. Every design decision maps to a governing principle:

| Design Decision | Governing Principle | Diagnostic Question |
|----------------|-------------------|-------------------|
| Which tools to assign | Ashby (variety) | Can this agent transcode all signal modes/formats it will encounter? |
| Which skills to authorize | Ashby (variety) | Does this agent have genre templates for its recurring encoding patterns? |
| What context_tier to set | Shannon (capacity) | How much context bandwidth does this agent need? |
| How to write Critical Rules | Ashby (attenuation) | What signals must this agent refuse to prevent competence overreach? |
| How to write Core Mission | Ashby (amplification) | What signal variety does this agent expand coverage for? |
| Who to set as reportsTo | Beer (topology) | Where do unresolvable signals route in the network? |
| What Success Metrics to define | Wiener (feedback) | How will alignment drift be detected over time? |

See `architecture/optimal-system-mapping.md` for the canonical Optimal System mapping.
